...
export interface PluginACardProps {
  item: PluginAItem;
  onClick?: () => void;
  onMouseEnter?: () => void;
  onMouseLeave?: () => void;
...
export const PluginACard: FC<PluginACardProps> = ({
  item,
  onClick,
  onMouseEnter,
  onMouseLeave,
...
export const SummaryCard: FC = () => (
  <Paper sx={{ p: 1 }}>
    <Typography variant="caption">{MOCK_PLUGINAS.length} items</Typography>
  </Paper>
);

/** Example: banner above/below map. Replace content or remove from plugin if not needed. */
export const PluginAAboveMapBanner: FC = () => (
  <Box sx={{ p: 0.5 }}>
    <Typography variant="caption">PluginA · {MOCK_PLUGINAS.length} items</Typography>
  </Box>
);
