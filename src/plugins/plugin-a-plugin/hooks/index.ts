...
import type { PluginAItem, PluginALayerMetadata } from "../types";

/** Example: returns mock data. Replace with real data source (API, Redux, etc.). */
export function usePluginAData() {
  return useMemo(() => ({ items: MOCK_PLUGINAS }), []);
}

/** Example: layer visibility and metadata. Adapt if you have multiple layers. */
export function usePluginALayer() {
  ...
  return useMemo(
    () => ({
      ...
      color: (mainLayer?.metadata as PluginALayerMetadata | undefined)?.color,
    }),
...
export function usePluginAHover() {
